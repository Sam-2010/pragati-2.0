...
export async function POST(req: Request) {
  try {
    ...
    4. Price limits: Check if the price seems reasonable for a ${subsidyReason}. For "Drip/Sprinkler Irrigation", the maximum acceptable receipt amount is Rs. 97,000 (or 90% of cost). Flag as "PRICE_MISMATCH" if it vastly exceeds this without justification.
    5. Initial Document Checks: Ensure Aadhaar shows proper ID. For 7/12 and 8A land records, verify land ownership: If the subsidy is "Drip/Sprinkler Irrigation", land ownership must be between 0.40 Ha and 5.0 Ha. For other subsidies, it must be between 0.20 Ha and 6.0 Ha. For Caste Certificate, ensure the caste is SC (Scheduled Caste) or Nav-Boudha. If any initial document violates these constraints, flag it.
